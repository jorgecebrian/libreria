...
// LóGICA PROGRAMA CAMBIO GIT para desktop ahora cambio en gitKraken
...
// PASSWORD
  ...
  function DesencriptaClave(const pClaveEncrip:String):String;
      // Devuelve clave encriptada o mensaje con error "Error: el car�cter X no se admite en el password."
  ...
  function ExtraeVersion (const pwdMezcla:String):String;
    // Dado un pwd Virosque mezclado extraigo solo opci�n
...
// TEXTO y FECHA y NUMERO #12 salto de p�gina, #10 salto de l�nea
...
//    Texto a Fecha
  ...
  function LimpiarCadenaDeCaracter(const pCadena:string; const pCaracter: Char): string;
     // Quita de una cadena todos los car�cteres que no sean letras o numeros
  function LimpiarCadenaSoloLetrasNumeros(const vCadena: string): string;
     // Quita de una cadena todos los car�cteres que no sean numeros
  function LimpiarCadenaSoloNumeros(const vCadena: string): string;
     // Genera una cadena con letras aleatorias (tama�o longitud)
  ...
  function FloatToSQLnum(const pNum:Double):String; // 23,5 ---> "23.5"
     // Posici�n de Primer car�cter no num�rico
  ...
  // API Windows
     // Devuelve el nombre de la m�quina y del usuario que ejecuta.
...
var
  ...
  i:Integer;
begin
  vLetrasOri  := 'L9A-Zza+Yb*rKs.Wd,oI8;_V&0Cl=){RyT[fXc]?�iE(G$3H�J%1M|x4NOP}QSuU!F6g2jk#mn��Bpe:qt@7hDv�5w';
  vLetrasDest := 'I�Bl4!e5�H9AgV�Ck.FUmjb+Nq)Kw{G?P6x0:r7=(fh�OQR]�EDno@8J,;3L2M1Sz_y-v*u}$t[s|p%i&ZaYcXdW#T';
  result := '';
...
function EncriptaClave(const  pClave:String):String;
var  // Devuelve clave encriptada o mensaje con error "Error: el car�cter X no se admite en el password."
  vLetrasOri, vLetrasDest:String;
  i, j:Integer;
begin
  vLetrasOri  := 'L9A-Zza+Yb*rKs.Wd,oI8;_V&0Cl=){RyT[fXc]?�iE(G$3H�J%1M|x4NOP}QSuU!F6g2jk#mn��Bpe:qt@7hDv�5w';
  ...
  for i := 1 to pClave.Length do begin
     ...
     if j=-1 then begin
       result := 'Error: el car�cter '+ pClave[i]+' no se admite en el password.';
       exit;
     end else
...
begin
    ...
    if viMes = 13 then begin     //Si nos hemos pasado, avanzamos el a�o.
      viMes := 1;
      viAno := viAno + 1;
    end;    //Y devolvemos el d�a anterior al primer d�a del mes siguiente.
...
function PosNoNum(const pTexto:String):Integer;
var      i: Integer; // Posici�n del primer car�cter no num�rico de un string
begin
...
begin             // Define la expresi�n regular para buscar n�meros en el texto
  vRegex := TRegEx.Create('\d+');
  // Busca la primera coincidencia de la expresi�n regular en el texto
...
// API WINDOWS
function GetCurrentUserName(out DomainName, UserName: string): Boolean;
var       // Devuelve el nombre de la m�quina y del usuario que ejecuta.
  Token: THandle;
